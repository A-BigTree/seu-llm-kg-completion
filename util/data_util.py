...
class ConvECorpus(Corpus):
    def __init__(self, args, train_data, val_data, test_data, entity2id, relation2id):
        ...
        self.val_head_indices = [
            {'triple': (tail, relation + rel_num, head), 'label': list(sr2o[(tail, relation + rel_num)])}
            for (head, relation, tail) in self.val_triples]
    ...
    def get_batch(self, batch_num):
        ...
        batch_indices = torch.LongTensor([indices['triple'] for indices in batch])
        label = [np.int32(indices['label']) for indices in batch]
